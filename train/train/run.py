...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    dataset = LocalTextCategorizationDataset(dataset_path, batch_size=train_conf['batch_size'], train_ratio=0.5, min_samples_per_label=train_conf['min_samples_per_label'], preprocess_text=embed) #**train_conf) , preprocess_text=embed
    logger.info(dataset)
    ...
    train_history = model.fit_generator(dataset.get_train_sequence(), epochs=train_conf['epochs'], verbose=train_conf['verbose'], validation_data= dataset.get_test_sequence())#, validation_data= dataset.get_test_sequence())
